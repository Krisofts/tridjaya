...
<x-ui.table
    title="CRM Leads"
    description="Kelola dan pantau semua prospek yang terdaftar di sistem."
>
    ...
    <thead class="border-gray-100 bg-gray-50 dark:border-gray-800 dark:bg-gray-900">
        <tr>
            ...
            <th class="px-6 py-3 text-left">
                <p class="text-theme-xs font-medium text-gray-500 dark:text-gray-400">Sumber</p>
            </th>
            <th class="px-6 py-3 text-left">
    ...
    <tbody class="divide-y divide-gray-100 dark:divide-white/[0.05]">
        ...
        @forelse ($leads as $lead)
            ...
            <tr class="cursor-pointer hover:bg-gray-50 dark:hover:bg-white/[0.02]" onclick="window.location='{{ route('crm.leads.show', $lead) }}'">
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                    @if ($lead->stage)
                        <x-ui.badge :type="$lead->stage->badgeType()">
                            {{ $lead->stage->name }}
                        </x-ui.badge>
                    @else
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                    @if ($lead->stage)
                        <x-ui.badge :type="$lead->stage->temperatureBadgeType()">
                    ...
                    <x-common.table-dropdown>
                        ...
                        <x-slot:content>
                            ...
                            <form
                                ...
                                method="POST"
                                onsubmit="return confirm('Yakin ingin menghapus lead {{ $lead->name }}?')"
                            >
                                @csrf
                                @method('DELETE')
                                <button
                                    type="submit"
                                    class="flex w-full rounded-lg px-3 py-2 text-left text-theme-xs font-medium text-red-500 hover:bg-red-50 hover:text-red-600"
                                >
                                    Hapus
                                </button>
                            </form>
        ...
        @empty
            ...
            <tr>
                <td colspan="8" class="px-6 py-12 text-center">
                    <p class="text-sm text-gray-500 dark:text-gray-400">Belum ada data prospek.</p>
